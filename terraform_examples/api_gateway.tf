...
resource "aws_api_gateway_resource" "event_gate_api_stats" {
  rest_api_id = aws_api_gateway_rest_api.event_gate_api.id
  parent_id = aws_api_gateway_rest_api.event_gate_api.root_resource_id
  path_part = "stats"
}

resource "aws_api_gateway_resource" "event_gate_api_stats_topic_name" {
  rest_api_id = aws_api_gateway_rest_api.event_gate_api.id
  parent_id = aws_api_gateway_resource.event_gate_api_stats.id
  path_part = "{topic_name}"
}

resource "aws_api_gateway_method" "event_gate_api_stats_topic_name_post" {
  rest_api_id = aws_api_gateway_rest_api.event_gate_api.id
...
resource "aws_api_gateway_integration" "event_gate_api_stats_topic_name_post_integration" {
  rest_api_id = aws_api_gateway_rest_api.event_gate_api.id
  resource_id = aws_api_gateway_resource.event_gate_api_stats_topic_name.id
...
resource "aws_lambda_permission" "event_stats_api_lambda_permissions" {
  action = "lambda:InvokeFunction"
  function_name = aws_lambda_function.event_stats_lambda.function_name
  principal = "apigateway.amazonaws.com"
  source_arn = "${aws_api_gateway_rest_api.event_gate_api.execution_arn}/*"
}
...
resource "aws_api_gateway_deployment" "event_gate_api_deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
	  ...
	  aws_api_gateway_integration.event_gate_api_stats_topic_name_post_integration
    ]))
  }
  lifecycle {
